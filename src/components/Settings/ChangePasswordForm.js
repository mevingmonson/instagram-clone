...
import { auth, credentials } from '../../lib/firebase';
...
class ChangePasswordForm extends Component {
  ...
  validateCurrentPassword = async () => {
    try {
      ...
      const userCredentials = credentials(user.email, this.state.currentPassword);
      ...
      <form className="change-pwd-form" onSubmit={this.handleSubmit}>
        ...
        <div className="form-group">
          ...
          {this.state.errorMessage && (
            <div>
              <span className="text-danger mr-2">{this.state.errorMessage}</span>
